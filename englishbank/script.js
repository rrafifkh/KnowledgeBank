(() => {
  ...
  const STORAGE_KEY = 'englishBank.words.v1';
  let words = loadWords();
  ...
  const elements = {
    ...
    search: $('#searchInput'), categoryFilter: $('#categoryFilter'), partOfSpeechFilter: $('#partOfSpeechFilter'), sort: $('#sortSelect'),
    dialog: $('#wordDialog'), form: $('#wordForm'), dialogTitle: $('#dialogTitle'), editId: $('#editId'),
    word: $('#wordInput'), category: $('#categoryInput'), partOfSpeech: $('#partOfSpeechInput'), meaning: $('#meaningInput'), notes: $('#notesInput'),
  ...
  function loadWords() {
    try {
      const stored = localStorage.getItem(STORAGE_KEY) ?? localStorage.getItem('languageBank.words.v1');
  ...
  function renderControls() {
    ...
    const partsOfSpeech = uniqueValues('partOfSpeech');
    updateSelect(elements.categoryFilter, categories, 'kategori');
    updateSelect(elements.partOfSpeechFilter, partsOfSpeech, 'jenis kata');
  ...
  function exportData() {
    ...
    link.href = url;
    link.download = `${STORAGE_KEY.split('.')[0]}-${new Date().toISOString().slice(0, 10)}.json`;
    link.click();
  ...
  function importData(event) {
    ...
    reader.onload = () => {
      try {
        ...
        const seen = new Set(words.map(item => canonicalWord(item.word)));
        const additions = valid.filter(item => !seen.has(canonicalWord(item.word)));
        additions.forEach(item => seen.add(canonicalWord(item.word)));
        words = [...words, ...additions];
        saveWords();
        render();
        showToast(`${additions.length} data baru berhasil ditambahkan.`);
      } catch { alert('File data tidak valid.'); }
      event.target.value = '';
    };
    reader.readAsText(file);
  }

  async function importBundledEnglishBackup() {
    const migrationKey = 'englishBank.importedBackup.2026-08-19';
    if (localStorage.getItem(migrationKey) === '1') return;
    try {
      let imported = window.LANGUAGE_BANK_SEED?.english;
      if (!Array.isArray(imported)) {
        const response = await fetch('../language-bank-backup-2026-08-19.json', { cache: 'no-store' });
        if (!response.ok) return;
        imported = (await response.json()).banks?.english;
      }
      if (!Array.isArray(imported)) return;
      const seen = new Set(words.map(item => canonicalWord(item.word)));
      const additions = imported
        .filter(item => item && item.id && item.word && !seen.has(canonicalWord(item.word)))
        .map(item => item.category ? item : { ...item, category: 'Dari Kamus' });
      additions.forEach(item => seen.add(canonicalWord(item.word)));
      words = [...words, ...additions];
      if (!saveWords()) return;
      localStorage.setItem(migrationKey, '1');
      render();
      showToast(`${additions.length} kosakata English berhasil dimasukkan.`);
    } catch {
      // Manual import remains available when the page is opened with file://.
    }
  }
  ...
  function visibleWords() {
    ...
    let result = words.filter(item => {
      const matchesQuery = !query || [item.word, item.meaning, item.category, item.partOfSpeech, item.notes].some(value => (value || '').toLocaleLowerCase('id').includes(query));
      return matchesQuery && (!elements.categoryFilter.value || item.category === elements.categoryFilter.value) && (!elements.partOfSpeechFilter.value || item.partOfSpeech === elements.partOfSpeechFilter.value);
    });
    const sorters = {
  ...
  function renderWords() {
    ...
    elements.wordGrid.innerHTML = result.length ? `
      <div class="table-scroll">
        <table class="word-table">
          <thead><tr>
            <th class="number-column">No.</th><th>Kosakata</th><th>Jenis Kata</th><th>Arti</th><th>Catatan</th><th class="date-column">Ditambahkan</th><th class="action-column">Aksi</th>
          </tr></thead>
          <tbody>${result.map((item, index) => `
            <tr class="word-card" data-id="${escapeHTML(item.id)}">
              <td class="number-cell">${index + 1}</td>
              <td class="word-cell">${escapeHTML(item.word)}</td>
              <td>${item.partOfSpeech ? escapeHTML(item.partOfSpeech) : '<span class="empty-value">—</span>'}</td>
    ...
    const hasFilters = elements.search.value || elements.categoryFilter.value || elements.partOfSpeechFilter.value;
  ...
  function openForm(item = null) {
    ...
    elements.dialogTitle.textContent = item ? 'Edit kosakata' : 'Tambah kosakata';
    if (item) {
      elements.word.value = item.word;
      elements.category.value = item.category || '';
      elements.partOfSpeech.value = item.partOfSpeech || '';
  ...
  function validateForm() {
    clearErrors();
    const fields = [
      [elements.word, 'Kosakata perlu diisi.'], [elements.meaning, 'Arti perlu diisi.']
    ];
    let valid = true;
  ...
  function submitForm(event) {
    ...
    if (duplicate) {
      elements.word.setCustomValidity('Kosakata ini sudah tersimpan.');
      elements.word.closest('.field').classList.add('invalid');
      elements.word.closest('.field').querySelector('.error').textContent = `Kosakata “${duplicate.word}” sudah tersimpan.`;
      elements.word.focus();
      elements.word.reportValidity();
      showToast('Kosakata yang sama sudah ada di English Bank.');
      return;
    }
    const existing = words.find(item => item.id === id);
    const item = {
      id: id || (crypto.randomUUID?.() || `${Date.now()}-${Math.random().toString(16).slice(2)}`),
      word: wordValue, category: normalized(elements.category.value), partOfSpeech: normalized(elements.partOfSpeech.value), meaning: normalized(elements.meaning.value),
    ...
    closeForm();
    render();
    showToast(id ? 'Kosakata berhasil diperbarui.' : 'Kosakata berhasil disimpan.');
  }
  ...
  function confirmDelete() {
    ...
    render();
    showToast('Kosakata telah dihapus.');
  }
  ...
  function showReviewCard() {
    ...
    elements.reviewPromptLabel.textContent = currentReviewFormat === 'type' ? 'ARTI' : 'KOSAKATA';
  ...
  [elements.search, elements.categoryFilter, elements.partOfSpeechFilter, elements.sort].forEach(control => control.addEventListener('input', renderWords));
