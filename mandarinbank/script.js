(() => {
  ...
  const STORAGE_KEY = 'mandarinBank.words.v1';
  let words = loadWords();
  ...
  const elements = {
    ...
    word: $('#wordInput'), pinyin: $('#pinyinInput'), category: $('#categoryInput'), meaning: $('#meaningInput'), notes: $('#notesInput'),
    ...
    reviewAnswer: $('#reviewAnswer'), reviewPinyin: $('#reviewPinyin'), reviewMeaning: $('#reviewMeaning'), reviewNotes: $('#reviewNotes'),
  ...
  function loadWords() {
    try {
      ...
      if (migratedWords.some((item, index) => item !== validWords[index])) {
  ...
  function canonicalWord(value) {
    return normalized(value).normalize('NFKC').replace(/\s/g, '');
  }
  ...
  async function restoreRecoveredData() {
    const recoveryKey = 'mandarinBank.recovered.2026-08-19';
    if (localStorage.getItem(recoveryKey) === '1') return;
    try {
      let recovered = window.LANGUAGE_BANK_SEED?.mandarin;
      if (!Array.isArray(recovered)) {
        const response = await fetch('../recovery/mandarin-recovered.json', { cache: 'no-store' });
        if (!response.ok) return;
        recovered = (await response.json()).words;
      }
      if (!Array.isArray(recovered)) return;
      const seen = new Set(words.map(item => canonicalWord(item.word)));
      const additions = recovered.filter(item => item?.word && !seen.has(canonicalWord(item.word)));
      additions.forEach(item => seen.add(canonicalWord(item.word)));
      words = [...words, ...additions];
      if (!saveWords()) return;
      localStorage.setItem(recoveryKey, '1');
      render();
      showToast(`${additions.length} kosakata Mandarin berhasil dipulihkan.`);
    } catch { /* Recovery remains available as JSON. */ }
  }

  function visibleWords() {
    const query = elements.search.value.trim().toLocaleLowerCase('id');
    let result = words.filter(item => {
      const matchesQuery = !query || [item.word, item.pinyin, item.meaning, item.category, item.notes].some(value => (value || '').toLocaleLowerCase('id').includes(query));
    ...
    elements.wordGrid.innerHTML = result.length ? `
      <div class="table-scroll">
        <table class="word-table">
          <thead><tr>
            <th class="number-column">No.</th><th>Hanzi</th><th>Pinyin</th><th>Arti</th><th>Catatan</th><th class="date-column">Ditambahkan</th><th class="action-column">Aksi</th>
          </tr></thead>
          <tbody>${result.map((item, index) => `
            <tr class="word-card" data-id="${escapeHTML(item.id)}">
              <td class="number-cell">${index + 1}</td>
              <td class="word-cell">${escapeHTML(item.word)}</td>
              <td class="pinyin-cell">${item.pinyin ? escapeHTML(item.pinyin) : '<span class="empty-value">—</span>'}</td>
  ...
  function openForm(item = null) {
    ...
    if (item) {
      ...
      elements.pinyin.value = item.pinyin || '';
  ...
  function validateForm() {
    clearErrors();
    const fields = [
      [elements.word, 'Hanzi perlu diisi.'], [elements.pinyin, 'Pinyin perlu diisi.'],
      [elements.meaning, 'Arti perlu diisi.']
    ];
    let valid = true;
  ...
  function submitForm(event) {
    ...
    if (duplicate) {
      elements.word.setCustomValidity('Hanzi ini sudah tersimpan.');
      elements.word.closest('.field').classList.add('invalid');
      elements.word.closest('.field').querySelector('.error').textContent = `Hanzi “${duplicate.word}” sudah tersimpan.`;
      elements.word.focus();
      elements.word.reportValidity();
      showToast('Hanzi yang sama sudah ada di Mandarin Bank.');
      return;
    }
    const existing = words.find(item => item.id === id);
    const item = {
      id: id || (crypto.randomUUID?.() || `${Date.now()}-${Math.random().toString(16).slice(2)}`),
      word: wordValue, pinyin: normalized(elements.pinyin.value),
      category: normalized(elements.category.value), meaning: normalized(elements.meaning.value),
  ...
  function showReviewCard() {
    ...
    elements.reviewPromptLabel.textContent = currentReviewFormat === 'type' ? 'ARTI' : 'HANZI';
    elements.reviewWord.textContent = currentReviewFormat === 'type' ? item.meaning : item.word;
    elements.reviewPinyin.textContent = item.pinyin || '—';
