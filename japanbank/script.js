(() => {
  ...
  const STORAGE_KEY = 'japanBank.words.v1';
  let words = loadWords();
  ...
  const elements = {
    ...
    word: $('#wordInput'), pinyin: $('#pinyinInput'), onyomi: $('#onyomiInput'), kunyomi: $('#kunyomiInput'), category: $('#categoryInput'), meaning: $('#meaningInput'), notes: $('#notesInput'),
  ...
  function visibleWords() {
    ...
    let result = words.filter(item => {
      const matchesQuery = !query || [item.word, item.pinyin, item.onyomi, item.kunyomi, item.meaning, item.category, item.notes].some(value => (value || '').toLocaleLowerCase('id').includes(query));
    ...
    elements.wordGrid.innerHTML = result.length ? `
      <div class="table-scroll">
        <table class="word-table">
          <thead><tr>
            <th class="number-column">No.</th><th>Kanji / Kana</th><th>Romaji</th><th>Onyomi</th><th>Kunyomi</th><th>Arti</th><th>Catatan</th><th class="date-column">Ditambahkan</th><th class="action-column">Aksi</th>
          </tr></thead>
          <tbody>${result.map((item, index) => `
            <tr class="word-card" data-id="${escapeHTML(item.id)}">
              ...
              <td class="reading-cell">${item.onyomi ? escapeHTML(item.onyomi) : '<span class="empty-value">—</span>'}</td>
              <td class="reading-cell">${item.kunyomi ? escapeHTML(item.kunyomi) : '<span class="empty-value">—</span>'}</td>
  ...
  function openForm(item = null) {
    ...
    if (item) {
      ...
      elements.onyomi.value = item.onyomi || '';
      elements.kunyomi.value = item.kunyomi || '';
  ...
  function validateForm() {
    clearErrors();
    const fields = [
      [elements.word, 'Kanji atau Kana perlu diisi.'], [elements.pinyin, 'Romaji perlu diisi.'],
  ...
  function submitForm(event) {
    ...
    if (duplicate) {
      elements.word.setCustomValidity('Kosakata Jepang ini sudah tersimpan.');
      ...
      showToast('Kosakata yang sama sudah ada di Japan Bank.');
      return;
    }
    ...
    const item = {
      ...
      onyomi: normalized(elements.onyomi.value), kunyomi: normalized(elements.kunyomi.value),
  ...
  function showReviewCard() {
    ...
    elements.reviewPromptLabel.textContent = currentReviewFormat === 'type' ? 'ARTI' : 'KANJI / KANA';
