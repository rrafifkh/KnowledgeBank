(() => {
  ...
  const STORAGE_KEY = 'politicalBank.words.v1';
  let words = loadWords();
    ...
    elements.wordGrid.innerHTML = result.length ? `
      <div class="table-scroll">
        <table class="word-table">
          <thead><tr>
            <th class="number-column">No.</th><th>Istilah Politik</th><th>Definisi</th><th>Catatan</th><th class="date-column">Ditambahkan</th><th class="action-column">Aksi</th>
          </tr></thead>
  ...
  function validateForm() {
    clearErrors();
    const fields = [
      [elements.word, 'Istilah politik perlu diisi.'], [elements.meaning, 'Definisi perlu diisi.']
    ];
    let valid = true;
  ...
  function submitForm(event) {
    ...
    if (duplicate) {
      ...
      showToast('Istilah yang sama sudah ada di Political Bank.');
      return;
    }
