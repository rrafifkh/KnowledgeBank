(() => {
  'use strict';
  const STORAGE_KEY = 'quoteBank.quotes.v1';
  ...
  const el = {
    ...
    title: $('#titleInput'), category: $('#categoryInput'), content: $('#contentInput'), notes: $('#notesInput'), suggestions: $('#categorySuggestions'),
    reader: $('#readerDialog'), readerTitle: $('#readerTitle'), readerCategory: $('#readerCategory'), readerDate: $('#readerDate'), readerContent: $('#readerContent'), readerNotes: $('#readerNotes'),
  ...
  function visibleStories() {
    ...
    const filtered = stories.filter(story => (!query || `${story.title} ${story.content}`.toLocaleLowerCase('id-ID').includes(query)) && (!el.categoryFilter.value || story.category === el.categoryFilter.value));
  ...
  function render() {
    ...
    el.grid.innerHTML=list.map((s,index)=>`<article class="story-card quote-card" data-id="${escapeHTML(s.id)}" tabindex="0" role="button"><span class="story-number" aria-label="Kutipan nomor ${index+1}">${String(index+1).padStart(2,'0')}</span><span class="story-bookmark" aria-hidden="true"></span><blockquote>“${escapeHTML(s.content.length>125?s.content.slice(0,125)+'…':s.content)}”</blockquote><h2>— ${escapeHTML(s.title)}</h2><span class="story-hint">Klik untuk membaca</span></article>`).join('');
  ...
  function openForm(story=null){el.form.reset();el.editId.value=story?.id||'';el.formTitle.textContent=story?'Edit kutipan':'Tambah kutipan';if(story){el.title.value=story.title;el.category.value=story.category||'';el.content.value=story.content;el.notes.value=story.notes||'';}el.formDialog.showModal();setTimeout(()=>el.title.focus(),30);}
  function submit(event){event.preventDefault();const title=clean(el.title.value),content=el.content.value.trim();$('#titleError').textContent=title?'':'Tokoh atau sumber perlu diisi.';$('#contentError').textContent=content?'':'Kutipan perlu diisi.';if(!title||!content)return;const id=el.editId.value;const duplicate=stories.find(s=>s.id!==id&&canonical(s.content)===canonical(content));if(duplicate){$('#contentError').textContent='Kutipan yang sama sudah ada.';el.content.focus();return;}const old=stories.find(s=>s.id===id);const story={id:id||(crypto.randomUUID?.()||`${Date.now()}`),title,category:clean(el.category.value),content,notes:el.notes.value.trim(),createdAt:old?.createdAt||new Date().toISOString(),updatedAt:new Date().toISOString()};stories=id?stories.map(s=>s.id===id?story:s):[story,...stories];if(save()){el.formDialog.close();render();showToast('Kutipan berhasil disimpan.');}}
  function openReader(id){const story=stories.find(s=>s.id===id);if(!story)return;activeId=id;el.readerTitle.textContent=`— ${story.title}`;el.readerCategory.textContent=story.category||'Tanpa kategori';el.readerDate.textContent=new Intl.DateTimeFormat('id-ID',{dateStyle:'long',timeStyle:'short'}).format(new Date(story.createdAt));el.readerContent.textContent=`“${story.content}”`;el.readerNotes.textContent=story.notes||'';el.readerNotes.hidden=!story.notes;el.reader.showModal();}
  function exportData(){const payload={version:1,exportedAt:new Date().toISOString(),storageKey:STORAGE_KEY,quotes:stories};const url=URL.createObjectURL(new Blob([JSON.stringify(payload,null,2)],{type:'application/json'}));const a=document.createElement('a');a.href=url;a.download=`quote-bank-${new Date().toISOString().slice(0,10)}.json`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);}
  function importData(event){const file=event.target.files[0];if(!file)return;const reader=new FileReader();reader.onload=()=>{try{const parsed=JSON.parse(reader.result);const incoming=Array.isArray(parsed)?parsed:parsed.quotes;if(!Array.isArray(incoming))throw new Error();const seen=new Set(stories.map(s=>canonical(s.content)));const additions=incoming.filter(s=>s?.title&&s?.content&&!seen.has(canonical(s.content)));additions.forEach(s=>seen.add(canonical(s.content)));stories=[...stories,...additions];save();render();showToast(`${additions.length} kutipan ditambahkan.`);}catch{alert('File Quote Bank tidak valid.');}event.target.value='';};reader.readAsText(file);}
  ...
  $('#deleteStoryButton').addEventListener('click',()=>{if(!activeId||!confirm('Hapus kutipan ini?'))return;stories=stories.filter(s=>s.id!==activeId);save();el.reader.close();render();showToast('Kutipan dihapus.');});
