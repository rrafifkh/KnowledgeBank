(() => {
  ...
  async function syncAll(){if(!session||syncing)return;if(!config.supabaseUrl||!config.supabaseKey)throw new Error('Konfigurasi Supabase belum tersedia.');syncing=true;setStatus('Menyinkronkan...');try{await ensureFreshSession();const rows=await api('/rest/v1/bank_snapshots?select=bank_key,payload,updated_at');const remote=new Map((rows||[]).map(row=>[row.bank_key,row.payload]));const payload=[];BANK_KEYS.forEach(key=>{const merged=merge(key,localData(key),remote.get(key));nativeSetItem.call(localStorage,key,JSON.stringify(merged));payload.push({user_id:session.user.id,bank_key:key,payload:merged,updated_at:new Date().toISOString()});});await api('/rest/v1/bank_snapshots?on_conflict=user_id,bank_key',{method:'POST',headers:{Prefer:'resolution=merge-duplicates,return=minimal'},body:JSON.stringify(payload)});cloudReady=true;setStatus('Sinkron selesai');window.dispatchEvent(new CustomEvent('knowledgebank:cloudsynced'));return payload;}finally{syncing=false;updateUI();}}
