(() => {
  ...
  const STORAGE_KEY = 'musicBank.words.v1';
  let words = loadWords();
  ...
  async function restoreRecoveredData() {
    const recoveryKey = 'musicBank.recovered.2026-08-19';
    if (localStorage.getItem(recoveryKey) === '1') return;
    try {
      let recovered = window.LANGUAGE_BANK_SEED?.music;
      if (!Array.isArray(recovered)) {
        const response = await fetch('../recovery/music-recovered.json', { cache: 'no-store' });
      ...
      render();
      showToast(`${additions.length} istilah Music berhasil dipulihkan.`);
    ...
    elements.wordGrid.innerHTML = result.length ? `
      <div class="table-scroll">
        <table class="word-table">
          <thead><tr>
            <th class="number-column">No.</th><th>Istilah Musik</th><th>Definisi</th><th>Catatan</th><th class="date-column">Ditambahkan</th><th class="action-column">Aksi</th>
          </tr></thead>
  ...
  function validateForm() {
    clearErrors();
    const fields = [
      [elements.word, 'Istilah musik perlu diisi.'], [elements.meaning, 'Definisi perlu diisi.']
    ];
    let valid = true;
  ...
  function submitForm(event) {
    ...
    if (duplicate) {
      ...
      showToast('Istilah yang sama sudah ada di Music Bank.');
      return;
    }
